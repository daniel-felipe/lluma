...
- Create dedicated Action classes for business logic with a single `run()` method.
...
- Some actions won't require dependencies via `__construct` and they can use just the `run()` method.
...
final readonly class CreateFavorite
{
    ...
    public function run(User $user, string $favorite): bool
    {
