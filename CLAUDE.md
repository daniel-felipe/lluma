...
# Run tests fast — filter by name or file
...
# PHP only (after modifying PHP files)
vendor/bin/pint --dirty --format agent

# Static analysis
vendor/bin/phpstan

# After adding/changing controllers or routes, regenerate Wayfinder types
php artisan wayfinder:generate
```

## Architecture

**Actions pattern**: All business logic lives in `app/Actions/`. Single `run()` method per class. Controllers are thin — they validate, call an action, redirect. Create via `php artisan make:action "{name}" --no-interaction`.

**Models**: All use UUIDs (`HasUuids`), strict typing, `final`, `readonly` where appropriate. Relationships are typed with generics in PHPDoc. Key models: `User` → `BarberProfile` → `Service`, `AvailabilitySlot`, `Appointment`.

**Enums**: `app/Enums/` — `BarberOnboardingStep`, `BarberProfileStatus`. TitleCase keys.

**Onboarding flow**: `BarberOnboardingStep` enum drives a multi-step wizard (phone → verify → profile → services → working-hours). `onboarding` middleware redirects incomplete barbers to their current step.

**Frontend**: Inertia pages in `resources/js/pages/`. Wayfinder-generated route functions in `resources/js/actions/` and `resources/js/routes/` — never hardcode URLs. shadcn/ui components (New York style) in `resources/js/components/ui/`.

**Specs**: Each feature has a spec in `specs/NNN-feature-name/` with `spec.md`, `plan.md`, `data-model.md`, `tasks.md`. Read the relevant spec before starting work on a feature branch.

**Database**: SQLite in dev/test. All schema changes via migrations. Check schema with `mcp__laravel-boost__database-schema` before writing migrations.
...
## Active Technologies
- PHP 8.5 / TypeScript 5 + Laravel 13, Inertia.js v3, React, Fortify v1, Pest v5, Tailwind v4, shadcn/ui (New York style) (001-barber-onboarding-profile)
- SQLite (dev/test), MySQL/PostgreSQL (production) — via Laravel migrations (001-barber-onboarding-profile)
- PHP 8.5, TypeScript 5 + Laravel 13, Inertia.js v3, React 19, Tailwind v4, Wayfinder v0, Pest v5, Larastan v3, shadcn/ui (New York) (002-service-catalog)
- SQLite (dev/test) via Laravel migrations — tabela `services` stub já existe; será expandida (002-service-catalog)
- PHP 8.5 + Laravel 13, Inertia.js v3, React 19, Tailwind v4, Wayfinder v0, Pest v5 (003-working-hours-config)

## Recent Changes
- 001-barber-onboarding-profile: Added PHP 8.5 / TypeScript 5 + Laravel 13, Inertia.js v3, React, Fortify v1, Pest v5, Tailwind v4, shadcn/ui (New York style)
